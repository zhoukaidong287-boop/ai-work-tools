...
- 客户总数：CRM 主数据客户总数量，死单客户也计入客户总数。
- 有效客户：非死单、非明确无效客户；沉睡客户仍属于有效客户。
- 死单客户：单独统计，不进入有效客户、今日待跟进和逾期未跟进。
- 新客户：优先按 `newCustomerDate === businessDate` 判断，`isNewCustomer` 仅作兼容字段。
- A/B/C 分类：`A类`、`A级`、`A客户`、`A类客户` 统一为 A；B/C 同理；A/B/C 统计默认不包含死单。
- 当前阶段 `currentStage` / `customerStage` / `status` 表示客户当前推进状态，例如新线索、已联系、已进店、已量房、已出方案、已报价、已交定金、已签单、沉睡客户、死单。
- 动作次数 `actionCounts` 表示实际发生过的动作，例如联系次数、进店次数、量房次数、出方案次数、报价次数、跟进次数、交定金次数、签单次数。
- 阶段和动作必须分离：不得把进店次数、量房次数、报价次数自动等同为客户当前阶段。
- 家装客户不是严格线性漏斗：允许未量房但已进店，允许量房过程中已报价，允许未进店但已沟通过报价区间。
- CRM、`report.html`、`city-analysis.html`、`index.html` 必须统一以上统计口径。
